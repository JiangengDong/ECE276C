...
class DDPG:
    def __init__(
        ...
        self.critic_target = Critic(state_dim=state_dim, action_dim=action_dim).cuda()
    ...
    def update_network(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Q_next = self.critic_target.forward(next_states, next_actions).squeeze()
        Q_backup = rewards + self.gamma * (1 - dones) * Q_next
        Q_predict = self.critic.forward(states, actions).squeeze()
        critic_loss = F.mse_loss(Q_predict, Q_backup)
        ...
        actor_loss = - self.critic.forward(states, actions).squeeze().mean()
...
if __name__ == "__main__":
    ...
    root_dir = "data/DDPG-0-shaping-rand"
    ...
    if task == "train":
        ...
        # Train the policy
        policy = DDPG_object.train(500000, log_dir, model_dir)
